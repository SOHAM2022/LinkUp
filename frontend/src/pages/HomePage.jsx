...
const HomePage = () => {
  ...
  console.log("HomePage data:", {
    friends: Array.isArray(friends) ? `Array(${friends.length})` : typeof friends,
    recommendedUsers: Array.isArray(recommendedUsers) ? `Array(${recommendedUsers.length})` : typeof recommendedUsers,
    loadingFriends,
    loadingUsers
  });
  ...
  const { mutate: sendRequestMutation, isPending } = useMutation({
    ...
    onSuccess: () => {
      // Invalidate queries for the sender
      queryClient.invalidateQueries({ queryKey: ["outgoingFriendReqs"] });
      // Note: The recipient's notifications will be updated by polling or when they navigate to notifications
    },
  });

  useEffect(() => {
    // Log any errors for debugging
    if (friendsError) console.error("Friends query error:", friendsError);
    if (usersError) console.error("Users query error:", usersError);
    if (outgoingError) console.error("Outgoing requests query error:", outgoingError);
  }, [friendsError, usersError, outgoingError]);
